...
public class TrackerRequestBuilder {
  ...
  public TrackerRequest createTrackerRequest(TorrentDownload download, TrackerRequest.Event event) {
    TrackerRequest request = new TrackerRequest(
        download.getInfoHash(),
        download.getTrackerRegisteredPeerId(),
        download.getPort(),
        event,
        download.getBytesUploaded(),
        download.getBytesDownloaded(),
        download.getBytesLeft());
    request.setCompactFormatWanted(true);
    request.setPeersWanted(200);
    return request;
  }
}
